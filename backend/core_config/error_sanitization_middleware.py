    ...
    def process_response(self, request, response):
        ...
        if hasattr(response, 'data') and isinstance(response.data, dict):
            if 'error' in response.data:
                # Already in standard format, just ensure it's sanitized
                sanitized_response = StandardErrorResponse(
                    error_code=response.data['error'].get('code', 'UNKNOWN_ERROR'),
                    message=response.data['error'].get('message', 'An error occurred'),
