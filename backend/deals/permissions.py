...
class HasPermission(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        elif viewset_name == 'DealCompatViewSet':
            required_perms_map = {
                'list': ['view_all_deals', 'view_own_deals'],
                'create': ['create_deal'],
                'retrieve': ['view_all_deals', 'view_own_deals'],
                ...
                'partial_update': ['update_deal_status'],
                'destroy': [], # Define if needed
                'update_status': ['update_deal_status'],
            }
        elif viewset_name == 'PaymentViewSet':
            required_perms_map = {
                # Salesperson can list & retrieve their payments with create_deal_payment; Verifier & OrgAdmin have verify_deal_payment
                'list': ['verify_deal_payment', 'view_all_deals', 'create_deal_payment'],
                'create': ['create_deal_payment'],  # allow salesperson
                ...
                'verify': ['verify_deal_payment'],  # custom action restricted to verifiers
            }
