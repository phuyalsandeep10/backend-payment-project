...
class BypassPreventionValidator(BaseValidator):
    ...
    def validate_file_comprehensive(self, file_obj, client_ip: str = None, 
        ...
        try:
            # Set context for behavioral analysis
    ...
    def _perform_malware_scan(self, file_obj, result: Dict):
        ...
        try:
            ...
            if scan_result.get('behavioral_analysis'):
                behavioral = scan_result['behavioral_analysis']
                if behavioral.get('behavioral_flags'):
                    result['bypass_attempts'].extend([
                        f"Behavioral: {flag}" for flag in behavioral['behavioral_flags']
                    ])
                
                # Update threat level based on behavioral risk
            ...
            # Add real-time threat results
            if scan_result.get('real_time_threats', {}).get('threat_detected'):
                real_time = scan_result['real_time_threats']
                result['bypass_attempts'].extend(real_time.get('threat_details', []))
