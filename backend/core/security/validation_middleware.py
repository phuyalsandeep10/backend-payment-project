...
class InputValidationMiddleware(MiddlewareMixin):
    ...
    EXEMPT_PATHS = [
        ...
        '/api/docs/',
        '/api/schema/',
        '/api/team/teams/',  # Exempt team endpoints from command injection validation
        '/api/auth/users/',   # Exempt user endpoints from command injection validation
        '/api/commission/',   # Exempt commission endpoints from command injection validation
        '/api/permissions/',  # Exempt permissions endpoints from command injection validation
        '/api/deals/',        # Exempt deals endpoints from overly aggressive validation
        '/api/clients/',      # Exempt clients endpoints from overly aggressive validation
    ]
    ...
    CSRF_EXEMPT_PATHS = [
        ...
        '/api/auth/change-password',  # Without trailing slash
        '/api/auth/password/set/',    # Non-existent endpoint that frontend tries first
        '/api/auth/password/set',     # Without trailing slash
        '/api/webhooks/',
    ]
    ...
    def process_request(self, request: HttpRequest) -> Optional[HttpResponse]:
        ...
        try:
            ...
            logger.warning(
                f"AttributeError in input validation middleware for {request.path}: {str(e)}",
                extra={
                    'path': request.path,
            ...
            # Continue without validation if attributes are missing
            return None
            
        except ValueError as e:
            # Handle value errors (e.g., JSON parsing issues)
            logger.warning(
                f"ValueError in input validation middleware for {request.path}: {str(e)}",
                extra={
                    'path': request.path,
                    'method': request.method,
                    'middleware': 'InputValidationMiddleware',
            ...
            logger.error(
                f"Unexpected error in input validation middleware for {request.path}: {type(e).__name__}",
                extra={
                    'path': request.path,
                    'method': request.method,
                    'middleware': 'InputValidationMiddleware',
    ...
    def _validate_csrf_token(self, request: HttpRequest) -> Optional[HttpResponse]:
        ...
        # Skip CSRF validation if using Token authentication
...
class SecurityHeadersMiddleware(MiddlewareMixin):
    ...
    def process_response(self, request: HttpRequest, response: HttpResponse) -> HttpResponse:
        ...
        try:
            ...
            # Handle cases where response doesn't support header setting
            logger.warning(
                f"AttributeError in SecurityHeadersMiddleware for {request.path}: {str(e)}",
                extra={
                    'path': request.path,
                    'method': request.method,
            ...
            logger.error(
                f"Error in SecurityHeadersMiddleware for {request.path}: {type(e).__name__}",
                extra={
                    'path': request.path,
                    'method': request.method,
                    'middleware': 'SecurityHeadersMiddleware',
...
class RateLimitMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request: HttpRequest) -> Optional[HttpResponse]:
        ...
        try:
            ...
            logger.warning(
                f"Redis connection error in rate limit middleware for {request.path}: {str(e)}",
                extra={
                    'path': request.path,
                    'method': request.method,
            ...
            # Handle any other errors gracefully
            logger.error(
                f"Error in rate limit middleware for {request.path}: {type(e).__name__}",
                extra={
                    'path': request.path,
                    'method': request.method,
                    'middleware': 'RateLimitMiddleware',
