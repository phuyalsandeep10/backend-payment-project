...
class CommissionViewSet(OptimizedQueryMixin, viewsets.ModelViewSet):
    """
    ...
    @action(detail=False, methods=['post'], url_path='bulk-calculate', permission_classes=[IsOrgAdminOrSuperAdmin])
    def bulk_calculate(self, request):
        """
        Optimized bulk commission calculation with caching
        """
        try:
    ...
    @action(detail=False, methods=['get'], url_path='reconciliation', permission_classes=[IsOrgAdminOrSuperAdmin])
    ...
    @action(detail=False, methods=['post'], url_path='fix-discrepancies', permission_classes=[IsOrgAdminOrSuperAdmin])
    def fix_discrepancies(self, request):
        """
    ...
    @action(detail=False, methods=['get'], url_path='analytics', permission_classes=[IsOrgAdminOrSuperAdmin])
    ...
    def audit_history(self, request, pk=None):
        ...
        try:
            ...
            # Get audit history
            history = CommissionAuditTrail.get_commission_audit_history(commission.id)
    ...
    @action(detail=False, methods=['post'], url_path='invalidate-cache', permission_classes=[IsOrgAdminOrSuperAdmin])
    def invalidate_cache(self, request):
        ...
        try:
            ...
            return Response({
                'message': 'Commission caches invalidated successfully',
                'organization_id': organization.id,
                'user_id': user_id
            }, status=status.HTTP_200_OK)
...
class OrgAdminCommissionView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            # Debug: Print all available roles to help identify the correct role name
            all_roles_in_org = User.objects.filter(organization=organization).exclude(role__isnull=True).values('role__name').distinct()
            print(f"DEBUG - Available roles in organization: {list(all_roles_in_org)}")
            
            salespeople = User.objects.filter(
                organization=organization,
                role__name__icontains='sales'  # Use icontains to be more flexible with role names
