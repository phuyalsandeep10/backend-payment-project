...
from apps.deals.models import Deal,Payment,PaymentInvoice
...
from apps.deals.serializers import PaymentApprovalSerializer,PaymentInvoiceSerializer,DealSerializer, PaymentSerializer
from apps.deals.models import PaymentApproval
...
def verifier_invoice_delete(request, invoice_id):
    try:
        ...
        invoice.delete()
        org_id = request.user.organization.id if request.user.organization else 'N/A'
        logger.info(f"User {request.user.id} deleted invoice {invoice_id} from organization {org_id}")
...
def verification_queue(request):
    ...
    try:
        ...
        pending_invoices = PaymentInvoice.objects.select_related(
            'deal__client', 
            'payment'
        ).filter(
            deal__organization=request.user.organization,
        ...
        if serializer.is_valid():
            try:
                ...
                return Response({
                    ...
                    'verified_at': approval_instance.approval_date.isoformat() if hasattr(approval_instance, 'approval_date') else None
