"""
Password Policy Service - Task 2.3.1
...
class PasswordPolicyService(BaseService):
    """
    Service for managing password policies and validation.
    Extracted from authentication.password_policy module.
    """
    ...
    def __init__(self, user=None, organization=None, **kwargs):
        ...
        self._common_passwords_cache = None

    def get_service_name(self) -> str:
        """Return the name of this service for logging and identification"""
        return "password_policy_service"

    def get_policy_for_organization(self, organization_id: Optional[int] = None) -> Dict[str, Any]:
        """
    ...
    def validate_password(self, password: str, user=None, organization_id: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            policy = self.get_policy_for_organization(organization_id)
            errors = []
            ...
            if user:
                if not policy['allow_username_in_password'] and hasattr(user, 'username') and user.username:
                    if user.username.lower() in password_lower:
                        errors.append("Password cannot contain your username")
                
                if not policy['allow_email_in_password'] and hasattr(user, 'email') and user.email:
            ...
            return {
                ...
                'errors': errors,
                'policy': policy,
                'strength_score': self.get_password_strength_score(password, user, organization_id)
            }
            
        except Exception as e:
            security_logger.error(f"Password validation error: {e}")
            return {
                'is_valid': False,
    ...
    def _is_common_password(self, password: str) -> bool:
        ...
        if self._common_passwords_cache is None:
            self._common_passwords_cache = [
                'password', '123456', '123456789', 'qwerty', 'abc123',
                'password123', 'admin', 'letmein', 'welcome', 'monkey',
                'dragon', 'master', 'shadow', 'qwerty123', 'password1'
            ]
        return password.lower() in self._common_passwords_cache

    def check_password_expiration(self, user) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            security_logger.error(f"Password expiration check error: {e}")
            return {'expired': False, 'expires_soon': False, 'error': str(e)}

    def generate_secure_password(self, organization_id: Optional[int] = None, length: int = 12) -> str:
        ...
        try:
            ...
            validation = self.validate_password(password, organization_id=organization_id)
            
            if not validation['is_valid']:
                # If somehow the generated password doesn't meet policy, try again
                return self.generate_secure_password(organization_id, length)
            
            return password
            
        except Exception as e:
            security_logger.error(f"Secure password generation error: {e}")
            # Return a basic secure password as fallback
            return secrets.token_urlsafe(length)

    def get_password_strength_score(self, password: str, user=None, organization_id: Optional[int] = None) -> int:
        ...
        try:
            ...
            max_repeated = max([password.count(c) for c in set(password)]) if password else 0
            if max_repeated > 2:
            ...
            security_logger.error(f"Password strength calculation error: {e}")
            return 0

    def check_password_reuse(self, user, new_password: str) -> bool:
        """
        ...
        try:
            ...
            security_logger.error(f"Password reuse check error: {e}")
            return False

    def add_password_to_history(self, user, password_hash: str) -> bool:
        ...
        try:
            ...
            security_logger.error(f"Password history add error: {e}")
            return False
